...
function ScrollButton(props: ScrollButtonProps) {
  ...
  const eventHandlers = React.useMemo<React.ComponentProps<"button">>(() => {
  ...
  const IconComp = (directionToIcon[direction] ?? ChevronUp) as React.ComponentType<React.SVGProps<SVGSVGElement>>;
  ...
  return (
    ...
    >
      {React.createElement(IconComp, { className: "size-4", "aria-hidden": true })}
    </button>
  );
}
