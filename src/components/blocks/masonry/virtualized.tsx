...
/**
 * Virtualized, balanced-column masonry layout.
 *
 * Unlike `Masonry` (CSS `columns-*`, no JS positioning), this variant
 * computes absolute item positions with a shortest-column-first packing
 * algorithm and only renders items within the scroll viewport (+ overscan).
 * Use it for large lists where CSS columns' "reading order" placement and
 * lack of virtualization become a real cost; otherwise prefer `Masonry`.
 */

import { Slot as SlotPrimitive } from "radix-ui";
import * as React from "react";
import { useComposedRefs } from "@/components/_internal/lib/compose-refs";
...
function useResizeObserver(positioner: Positioner) {
  ...
  // eslint-disable-next-line react-hooks/preserve-manual-memoization -- custom WeakMap-based memoization the compiler cannot model
...
const ROOT_NAME = "VirtualMasonry";
const VIEWPORT_NAME = "VirtualMasonryViewport";
const ITEM_NAME = "VirtualMasonryItem";
...
type RootElement = React.ComponentRef<typeof VirtualMasonry>;
type ItemElement = React.ComponentRef<typeof VirtualMasonryItem>;
...
interface VirtualMasonryProps extends DivProps {
  columnWidth?: number;
  columnCount?: number;
  maxColumnCount?: number;
...
function VirtualMasonry(props: VirtualMasonryProps) {
  const {
    columnWidth = COLUMN_WIDTH,
    columnCount,
    maxColumnCount,
  ...
  return (
    <MasonryContext.Provider value={contextValue}>
      ...
      >
        <VirtualMasonryViewport>{children}</VirtualMasonryViewport>
      </RootPrimitive>
    </MasonryContext.Provider>
  );
}

interface MasonryItemPropsWithRef extends VirtualMasonryItemProps {
  ref: React.Ref<ItemElement | null>;
}

function VirtualMasonryViewport(props: DivProps) {
  ...
  const validChildren = React.Children.toArray(children).filter(
    (child): child is React.ReactElement<MasonryItemPropsWithRef> =>
      React.isValidElement(child) && child.type === VirtualMasonryItem,
  );
...
interface VirtualMasonryItemProps extends DivProps {
  asChild?: boolean;
}

function VirtualMasonryItem(props: VirtualMasonryItemProps) {
  const { asChild, ref, ...itemProps } = props;

  const ItemPrimitive = asChild ? SlotPrimitive.Slot : "div";

  return (
    <ItemPrimitive data-slot="virtual-masonry-item" {...itemProps} ref={ref} />
  );
}

export { VirtualMasonry, VirtualMasonryItem, type VirtualMasonryProps };
