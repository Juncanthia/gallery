...
export type FluidGlassProps = {
  mode?: "lens" | "bar" | "cube"
  lensProps?: { navItems?: NavItem[] } & Record<string, any>
  barProps?: { navItems?: NavItem[] } & Record<string, any>
  cubeProps?: { navItems?: NavItem[] } & Record<string, any>
}
...
function Typography() {
  const DEVICE: Record<string, { max: number; fontSize: number }> = {
    mobile: { max: 639, fontSize: 0.2 },
    tablet: { max: 1023, fontSize: 0.4 },
    desktop: { max: Infinity, fontSize: 0.6 },
  }
  const getDevice = () => {
...
export function FluidGlass({ mode = "lens", lensProps = {}, barProps = {}, cubeProps = {} }: FluidGlassProps) {
