...
class App {
  ...
  constructor(container: HTMLElement, {
    ...
    this.onCheckDebounce = debounce(this.onCheck, 200).bind(this)
    this.createRenderer()
    this.createCamera()
    this.createScene()
    this.onResize()
  ...
  createMedias(items: any, bend = 1, textColor: string, borderRadius: number, font: string) {
    ...
    this.medias = this.mediasImages.map((data: any, index: number) => {
      return new Media({
        ...
        gl: this.gl, image: data.image, index,
...
export function CircularGallery({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      app = new App(containerRef.current, { items, bend, textColor, borderRadius, font: font, scrollSpeed, scrollEase })
    }
    init()
