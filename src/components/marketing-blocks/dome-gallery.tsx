...
export function DomeGallery({
    ...
    const close = () => {
      ...
      const cleanup = () => {
        ...
        requestAnimationFrame(() => {
          el.style.visibility = ""
          el.style.opacity = "0"; el.style.zIndex = "0"
          focusedElRef.current = null
          rootRef.current?.removeAttribute("data-enlarging")
          requestAnimationFrame(() => {
  ...
  const openItemFromElement = useCallback((el: HTMLElement) => {
    ...
    overlay.style.cssText = `position:absolute;left:${frameR.left - mainR.left}px;top:${frameR.top - mainR.top}px;width:${frameR.width}px;height:${frameR.height}px;opacity:0;z-index:30;transform-origin:top left;transition:transform ${enlargeTransitionMs}ms ease,opacity ${enlargeTransitionMs}ms ease`
    const rawSrc = parent.dataset.src || el.querySelector("img")?.src || ""
    const img = document.createElement("img"); img.src = rawSrc; overlay.appendChild(img)
    viewerRef.current?.appendChild(overlay)
    const tx0 = tileR.left - frameR.left; const ty0 = tileR.top - frameR.top
    const sx0 = tileR.width / frameR.width; const sy0 = tileR.height / frameR.height
    ...
    if (wantsResize) {
      const onFirstEnd = (ev: TransitionEvent) => {
        ...
        const tempWidth = openedImageWidth || `${frameR.width}px`; const tempHeight = openedImageHeight || `${frameR.height}px`
        ...
        requestAnimationFrame(() => { overlay.style.left = `${centeredLeft}px`; overlay.style.top = `${centeredTop}px`; overlay.style.width = tempWidth; overlay.style.height = tempHeight })
      }
