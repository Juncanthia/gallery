...
export function GridDistortion({
  ...
  useEffect(() => {
    ...
    const handleResize = () => {
      ...
      if (plane) plane.scale.set(width / height, 1, 1)
      camera.left = -(width / height) / 2; camera.right = (width / height) / 2; camera.top = 0.5; camera.bottom = -0.5
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      mouseState.vX = x - mouseState.prevX; mouseState.vY = y - mouseState.prevY
    ...
    return () => {
      ...
      if (renderer) { renderer.dispose(); renderer.forceContextLoss(); if (container.contains(renderer.domElement)) container.removeChild(renderer.domElement) }
      geometry.dispose(); material.dispose(); dataTexture.dispose(); uniforms.uTexture.value?.dispose()
      sceneRef.current = null; rendererRef.current = null; cameraRef.current = null; planeRef.current = null
    }
