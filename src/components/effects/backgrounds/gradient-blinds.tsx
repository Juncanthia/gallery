...
void mainImage( out vec4 fragColor, in vec2 fragCoord ) {
    ...
    float d = length(uv0 - offset); float r = max(uSpotlightRadius, 1e-4);
    float dn = d / r;
...
export function GradientBlinds({
  ...
  useEffect(() => {
    ...
    const uniforms: any = {
      ...
      uBlindCount: { value: Math.max(1, blindCount) }, uSpotlightRadius: { value: spotlightRadius },
      uSpotlightSoftness: { value: spotlightSoftness }, uSpotlightOpacity: { value: spotlightOpacity },
    ...
    const geometry = new Triangle(gl); geometryRef.current = geometry
    const mesh = new Mesh(gl, { geometry, program }); meshRef.current = mesh
    ...
    const resize = () => {
      ...
      } else { uniforms.uBlindCount.value = Math.max(1, blindCount) }
      if (firstResizeRef.current) {
        firstResizeRef.current = false
        const cx = gl.drawingBufferWidth / 2, cy = gl.drawingBufferHeight / 2
        uniforms.iMouse.value = [cx, cy]; mouseTargetRef.current = [cx, cy]
      }
    }
    resize()
    ...
    const onPointerMove = (e: PointerEvent) => {
      const rect = canvas.getBoundingClientRect(); const scale = renderer.dpr || 1
      const x = (e.clientX - rect.left) * scale; const y = (rect.height - (e.clientY - rect.top)) * scale
  ...
  return <div ref={containerRef} className={className} style={{ ...(mixBlendMode && { mixBlendMode }) }} />
}
