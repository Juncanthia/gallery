...
export function FloatingLines({
  ...
  useEffect(() => {
    ...
    if (linesGradient && linesGradient.length > 0) {
      ...
      stops.forEach((hex, i) => { const color = hexToVec3(hex); uniforms.lineGradient.value[i].set(color.x, color.y, color.z) })
    }
    ...
    const handlePointerMove = (event: PointerEvent) => {
      ...
      if (parallax) {
        ...
        const offsetX = (x - centerX) / rect.width, offsetY = -(y - centerY) / rect.height
    ...
    return () => {
      ...
      if (interactive) { renderer.domElement.removeEventListener("pointermove", handlePointerMove); renderer.domElement.removeEventListener("pointerleave", handlePointerLeave) }
