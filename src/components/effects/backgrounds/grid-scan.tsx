...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
  ...
  if (uLineStyle > 0.5) {
    float dashRepeat = 4.0, dashDuty = 0.5;
    float vy = fract(gridUV.y * dashRepeat), vx = fract(gridUV.x * dashRepeat);
    float dashMaskY = step(vy, dashDuty), dashMaskX = step(vx, dashDuty);
    if (uLineStyle < 1.5) { lineX *= dashMaskY; lineY *= dashMaskX; }
    else { float dotRepeat = 6.0, dotWidth = 0.18;
      float cy = abs(fract(gridUV.y * dotRepeat) - 0.5), cx = abs(fract(gridUV.x * dotRepeat) - 0.5);
  ...
  if (uLineStyle > 0.5) {
    float dashRepeat2 = 4.0, dashDuty2 = 0.5;
    float vy2m = fract(gridUV2.y * dashRepeat2), vx2m = fract(gridUV2.x * dashRepeat2);
    float dashMaskY2 = step(vy2m, dashDuty2), dashMaskX2 = step(vx2m, dashDuty2);
    if (uLineStyle < 1.5) { lineX2 *= dashMaskY2; lineY2 *= dashMaskX2; }
    else { float dotRepeat2 = 6.0, dotWidth2 = 0.18;
      float cy2 = abs(fract(gridUV2.y * dotRepeat2) - 0.5), cx2 = abs(fract(gridUV2.x * dotRepeat2) - 0.5);
...
function smoothDampVec2(current: THREE.Vector2, target: THREE.Vector2, currentVelocity: THREE.Vector2, smoothTime: number, _maxSpeed: number, deltaTime: number) {
  ...
  if (origMinusCurrent.dot(outMinusOrig) > 0) { out.copy(originalTo); currentVelocity.set(0, 0) }
  return out
}

function smoothDampFloat(current: number, target: number, velRef: { v: number }, smoothTime: number, _maxSpeed: number, deltaTime: number) {
  const st = Math.max(0.0001, smoothTime)
  const omega = 2 / st
  const x = omega * deltaTime
  const exp = 1 / (1 + x + 0.48 * x * x + 0.235 * x * x * x)
...
export function GridScan({ enableWebcam = false, showPreview = false, sensitivity = 0.55, lineThickness = 1, linesColor = "#2F293A", scanColor = "#FF9FFC", scanOpacity = 0.4, gridScale = 0.1, lineStyle = "solid", lineJitter = 0.1, scanDirection = "pingpong", enablePost = true, bloomIntensity = 0, bloomThreshold = 0, bloomSmoothing = 0, chromaticAberration = 0.002, noiseIntensity = 0.01, scanGlow = 0.5, scanSoftness = 2, scanPhaseTaper = 0.9, scanDuration = 2.0, scanDelay = 2.0, className, style }: GridScanProps) {
  ...
  useEffect(() => {
    ...
    const uniforms: any = {
      ...
      uLinesColor: { value: srgbColor(linesColor) }, uScanColor: { value: srgbColor(scanColor) },
      ...
      uBloomOpacity: { value: bloomIntensity }, uScanGlow: { value: scanGlow },
