...
export type HyperspeedProps = {
  effectOptions?: Partial<typeof DEFAULT_EFFECT_OPTIONS> & {
    colors?: Partial<typeof DEFAULT_EFFECT_OPTIONS.colors>
  }
}
...
export function Hyperspeed({ effectOptions = DEFAULT_EFFECT_OPTIONS }: HyperspeedProps) {
    ...
    class CarLights {
      ...
      init() {
        ...
        for (let i = 0; i < options.lightPairsPerRoadWay; i++) {
          ...
          const laneX = carLane * laneWidth - options.roadWidth / 2 + laneWidth / 2

          const carWidth = random(options.carWidthPercentage) * laneWidth
          const carShiftX = random(options.carShiftX) * laneWidth
          const adjustedLaneX = laneX + carShiftX

          const offsetY = random(options.carFloorSeparation) + radius * 1.3

          const offsetZ = -random([0, options.length])

          aOffset.push(adjustedLaneX - carWidth / 2)
  ...
  return <div id="lights" ref={hyperspeed} className="w-full h-full overflow-hidden absolute" />
}
