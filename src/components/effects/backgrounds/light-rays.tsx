...
uniform vec2 rayPos;uniform vec2 rayDir;uniform vec3 raysColor;uniform float raysSpeed;
uniform float lightSpread;uniform float rayLength;uniform float pulsating;
uniform float fadeDistance;uniform float saturation;
...
float rayStrength(vec2 raySource,vec2 rayRefDirection,vec2 coord,float seedA,float seedB,float speed){
  ...
  float lengthFalloff=clamp((maxDistance-distance)/maxDistance,0.,1.);
  ...
  float baseStrength=clamp((0.45+0.15*sin(distortedAngle*seedA+iTime*speed))+(0.3+0.2*cos(-distortedAngle*seedB+iTime*speed)),0.,1.);
...
void main(){vec4 color;mainImage(color,gl_FragCoord.xy);gl_FragColor=color;}`
      ...
      const updatePlacement = () => {
        ...
        const w = wCSS * dpr
        const h = hCSS * dpr
        ;(uniforms.iResolution.value as number[])[0] = w
        ;(uniforms.iResolution.value as number[])[1] = h
        const { anchor, dir } = getAnchorAndDir(raysOrigin, w, h)
        ;(uniforms.rayPos.value as number[])[0] = anchor[0]
        ;(uniforms.rayPos.value as number[])[1] = anchor[1]
        ;(uniforms.rayDir.value as number[])[0] = dir[0]
        ;(uniforms.rayDir.value as number[])[1] = dir[1]
      }
      ...
      const loop = (t: number) => {
        ...
        if (followMouse && mouseInfluence > 0.0) {
          ...
          ;(uniforms.mousePos.value as number[])[0] = smoothMouseRef.current.x
          ;(uniforms.mousePos.value as number[])[1] = smoothMouseRef.current.y
        }
        try {
