...
vec3 hash3(uint n) { uvec3 hashed = hash(n) * uvec3(1U, 511U, 262143U); return vec3(hashed) * F0; }
  ...
  for (int i = 0; i < 128; i++) {
    ...
    if (cellHash < uDensity) {
      ...
      if (toIntersection > 0.0) {
        ...
        else { float invFlakeSize = 1.0 / flakeSize; dist = snowflakeDist(vec2(testX, testY) * invFlakeSize) * flakeSize; }
        if (dist < flakeSize) {
