...
export function Plasma({
  ...
  useEffect(() => {
    ...
    const handleContextLost = (e: Event) => { e.preventDefault(); contextLost = true; cancelAnimationFrame(raf) }
    const handleContextRestored = () => { contextLost = false; if (isVisible) { cancelAnimationFrame(raf); raf = requestAnimationFrame(loop) } }
    ...
    const io = new IntersectionObserver(([entry]) => {
      ...
      if (isVisible && !wasVisible && !contextLost) { cancelAnimationFrame(raf); raf = requestAnimationFrame(loop) }
    }, { threshold: 0 })
