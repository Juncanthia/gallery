...
export function Shuffle({
  ...
  // Custom character split helper (avoiding GSAP SplitText dependency)
  useGSAP(
    () => {
      ...
      const charElements = Array.from(el.children) as HTMLSpanElement[]
      wrappersRef.current = []
      ...
      charElements.forEach((ch) => {
        ...
        const firstOrig = ch.cloneNode(true) as HTMLSpanElement
        ...
        for (let k = 0; k < rolls; k++) {
          ...
          Object.assign((c as HTMLSpanElement).style, {
            display: shuffleDirection === "up" || shuffleDirection === "down" ? "block" : "inline-block",
            width: w + "px",
            textAlign: "center",
          })
  ...
  const classes = useMemo(
    () => cn("inline-block whitespace-normal break-words leading-none text-4xl font-['Press_Start_2P',sans-serif] uppercase", ready ? "visible" : "invisible", className),
    [ready, className]
  )
