...
const createTouchTexture = () => {
  ...
  const addTouch = (norm: { x: number; y: number }) => {
    ...
    if (last) {
      ...
      vx = dx / (d || 1); vy = dy / (d || 1); force = Math.min(dd * 10000, 1)
    }
...
uniform float uScale; uniform float uDensity; uniform float uPixelJitter;
...
void main(){
  ...
  float speed=uRippleSpeed,thickness=uRippleThickness;
  if(uEnableRipples==1){
    for(int i=0;i<MAX_CLICKS;++i){
      ...
      float waveR=speed*t,ring=exp(-pow((r-waveR)/thickness,2.));
      float atten=exp(-t)*exp(-10.*r);
  ...
  if(uEdgeFade>0.){vec2 norm=gl_FragCoord.xy/uResolution;float edge=min(min(norm.x,norm.y),min(1.-norm.x,1.-norm.y));float fade=smoothstep(0.,uEdgeFade,edge);M*=fade;}
  vec3 color=uColor;
  vec3 srgbColor=mix(color*12.92,1.055*pow(color,vec3(1./2.4))-.055,step(.0031308,color));
...
export function PixelBlast({
  ...
  useEffect(() => {
    ...
    if (mustReinit) {
      ...
      const mapToPixels = (e: PointerEvent) => {
        ...
        return { fx: (e.clientX - rect.left) * renderer.domElement.width / rect.width, fy: (rect.height - (e.clientY - rect.top)) * renderer.domElement.height / rect.height, w: renderer.domElement.width, h: renderer.domElement.height }
      }
      ...
      t.uniforms.uRippleThickness.value = rippleThickness; t.uniforms.uRippleSpeed.value = rippleSpeed
  ...
  return <div ref={containerRef} className={`w-full h-full ${className ?? ""}`} style={style} aria-label="PixelBlast interactive background" />
}
