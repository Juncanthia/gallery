...
function main() {
  ...
  const existingCount = existsSync(OUT_FILE)
    ? (readFileSync(OUT_FILE, "utf8").match(/^\s+item\(\{/gm) ?? []).length
    : 0

  if (entries.length === 0 && existingCount > 0) {
    console.log(
      `No new pure-reexport shells; preserving ${existingCount} existing entries in ${OUT_FILE}`
    )
    console.log("Run pnpm sync:registry-paths to align file paths with content/")
    return
  }
